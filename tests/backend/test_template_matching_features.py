...
def test_snap_bboxes_tightens_offset_box():
    """Snap should tighten a detection box to the actual ink."""
    # Place a circle at (105, 35) but give the detection a deliberately
    # offset box that's shifted 5px right and 5px down.
    img = np.full((200, 400), 255, dtype=np.uint8)
    cv2.circle(img, (115, 55), 8, 0, -1)  # ink centered at (115, 55)

    staff = _make_staff()
    # Template: circle with extra padding on the left (asymmetric)
    template = np.full((40, 30), 255, dtype=np.uint8)
    cv2.circle(template, (20, 20), 8, 0, -1)  # off-center in template
    ...
    ctx = PipelineContext(
        image=img,
        staves=[staff],
        config={"confidence_threshold": 0.4, "snap_bboxes": True},
    )
    result = stage.process(ctx)

    assert len(result.symbols) > 0
    sym = result.symbols[0]
    # The snapped box should contain the circle center (115, 55)
    assert sym.x <= 115 <= sym.x + sym.width
    assert sym.y <= 55 <= sym.y + sym.height


def test_snap_bboxes_disabled():
    """When snap_bboxes is False, boxes should stay as matchTemplate returned."""
    ...
    ctx = PipelineContext(
        image=img,
        staves=[staff],
        config={"confidence_threshold": 0.5, "snap_bboxes": False},
    )
    result = stage.process(ctx)

    assert len(result.symbols) > 0
    # Box dimensions should exactly equal the (scaled) template dimensions
    sym = result.symbols[0]
    assert sym.width == template.shape[1]
    assert sym.height == template.shape[0]
