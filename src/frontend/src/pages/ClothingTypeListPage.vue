...
async function load() {
  items.value = await get("/clothing-types");
}
...
async function save() {
  try {
    if (editing.value === "new") {
      await post("/clothing-types", form.value);
    } else {
      await put(`/clothing-types/${editing.value}`, form.value);
    }
    editing.value = null;
    await load();
  } catch (e) {
...
async function remove() {
  try {
    await del(`/clothing-types/${deleteTarget.value}`);
...
<template>
  <div>
    ...
    <ConfirmDialog
      :open="!!deleteTarget"
      title="Typ löschen"
      message="Soll dieser Kleidungstyp wirklich gelöscht werden?"
      @confirm="remove"
